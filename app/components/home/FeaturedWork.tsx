...
function hasTag(gallery: Gallery, tag: string): boolean {
...
function getVerticalOffset(id: string, index: number): number {
...
type FeedItem =
  | { type: "gallery"; data: Gallery; order: number }
  | { type: "quote"; data: Quote; order: number };

export default function FeaturedWork({ galleries, quotes = [] }: FeaturedWorkProps) {
  ...
  const verticalOffsets = useMemo(() => {
    return feedItems.map((item, i) =>
      getVerticalOffset(item.type === "gallery" ? item.data.id : `quote-${i}`, i)
    );
  }, [feedItems]);
